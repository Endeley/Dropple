...
export function getEasingFn(ease) {
  ...
  if (typeof ease === "object" && ease.type === "bezier") {
    const bezIn = ease.in || { x: 0.25, y: 0.1 };
    const bezOut = ease.out || { x: 0.25, y: 1 };
    return cubicBezierEase([bezIn.x, bezIn.y, bezOut.x, bezOut.y]);
  }
