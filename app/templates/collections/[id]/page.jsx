...
export default function CollectionDetailPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const match = (data.collections || []).find((c) => c._id === id);
  ...
  useEffect(() => {
    if (!collection?.templateIds?.length) return;
    const load = async () => {
      try {
        const res = await fetch("/api/templates/marketplace");
        const data = await res.json();
        const templ = (data.templates || []).filter((t) => collection.templateIds.includes(t.id));
        setTemplates(templ);
      } catch (err) {
        console.error("Failed to load templates for collection", err);
      }
    };
    load();
  }, [collection]);
        ...
        <div className="flex items-start gap-4">
          <div className="relative h-20 w-20 rounded-lg border border-neutral-200 bg-neutral-100 overflow-hidden">
            {collection?.thumbnailUrl ? (
              <Image src={collection.thumbnailUrl} alt={collection.name} fill className="object-cover" sizes="120px" />
            ) : (
              <div className="w-full h-full bg-gradient-to-br from-neutral-100 to-neutral-200" />
            )}
          </div>
          <div>
            <div className="text-xl font-semibold text-neutral-900">{collection?.name || "Collection"}</div>
            <div className="text-sm text-neutral-500">
              {(collection?.templateIds || []).length} templates {collection?.curated ? "· Curated" : ""}
            </div>
          </div>
        </div>
