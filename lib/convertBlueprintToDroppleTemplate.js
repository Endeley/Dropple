...
export function convertBlueprintToDroppleTemplate(bp = {}) {
  ...
  const assets = normalizeAssets(
    (bp.images || []).map((img, idx) => ({
      id: img.id || `asset_${idx}`,
      type: "image",
      url: img.url || img.src || placeholderImage(img.id || idx),
      prompt: img.prompt,
      source: img.source || classifySource(img.url),
      license: img.license || classifyLicense(img.url),
    })),
  );
...
function convertNodesBlueprint(bp = {}) {
  ...
  (bp.nodes || []).forEach((n, idx) => {
    ...
    if (n.type === "text") {
      ...
      nodeImageAssets.push({
        id: n.imageId || id,
        type: "image",
        url: resolveImageUrl(n, imagesMap, idx),
        prompt: n.prompt,
        source: n.source || classifySource(n.url || imagesMap[n.imageId]),
        license: n.license || classifyLicense(n.url || imagesMap[n.imageId]),
      });
  ...
  const baseAssets =
    Array.isArray(bp.images) && bp.images.length
      ? bp.images.map((img, idx) => ({
          id: img.id || `asset_${idx}`,
          type: img.type || "image",
          url: img.url || img.src || placeholderImage(img.id || idx),
          prompt: img.prompt,
          source: img.source || classifySource(img.url || img.src),
          license: img.license || classifyLicense(img.url || img.src),
        }))
      : [];

  const assets = normalizeAssets([...baseAssets, ...nodeImageAssets]);
...
function classifySource(url = "") {
  ...
  if (url.startsWith("/packs/")) return "pack";
  if (url.includes("pexels.com") || url.includes("unsplash.com")) return "stock";
...
function classifyLicense(url = "") {
  ...
  if (url.startsWith("/packs/")) return "pexels";
  return "unknown";
}
