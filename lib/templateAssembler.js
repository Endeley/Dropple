...
export async function assembleTemplate({
  ...
  const { nodes, animations, images } = buildLayout(layout.id, style, content, brand);
...
function buildLayout(layoutId, style, content, brand) {
  switch (layoutId) {
    case "promoBanner":
      return buildPromoBanner(style, content, brand);
    case "youtubeThumbnail":
      return buildYouTubeThumb(style, content, brand);
    case "gradientQuote":
      return buildGradientQuote(style, content, brand);
    case "materialCard":
      return buildMaterialCard(style, content, brand);
    case "eventFlyer":
      return buildEventFlyer(style, content, brand);
    case "businessCard":
      return buildBusinessCard(style, content, brand);
    case "podcastCover":
      return buildPodcastCover(style, content, brand);
    default:
      return buildHeroStory(style, content, brand);
  }
}

function buildHeroStory(style, content, brand) {
  ...
  const heroUrl = pickImage(content, brand);
  const nodes = [
...
function buildPromoBanner(style, content, brand) {
  const palette = style?.palette || {};
  ...
  const images = [
    { id: "img1", prompt: content.imagePrompt || "product hero, dramatic studio lighting, soft shadows", url: heroUrl },
  ];

  return { nodes, animations, images };
}

function buildYouTubeThumb(style, content, brand) {
  const palette = style?.palette || {};
  ...
  const images = [{ id: "img1", prompt: content.imagePrompt || "cinematic tech scene, neon, depth of field", url: heroUrl }];
  return { nodes, animations, images };
}

function buildGradientQuote(style, content, brand) {
  const palette = style?.palette || {};
  const primary = palette.primary || "#8B5CF6";
...
function buildMaterialCard(style, content, brand) {
  const palette = style?.palette || {};
  const surface = palette.surface || "#ffffff";
  const primary = palette.primary || "#2563EB";
...
function buildEventFlyer(style, content, brand) {
  const palette = style?.palette || {};
  const primary = palette.primary || "#EF4444";
  const accent = palette.accent || "#F97316";
  const text = palette.text || "#F8FAFC";
  ...
  const images = [{ id: "img1", prompt: content.imagePrompt || "cinematic portrait, dramatic lighting, moody atmosphere", url: heroUrl }];
  return { nodes, animations, images };
}

function buildBusinessCard(style, content, brand) {
  const palette = style?.palette || {};
  const surface = palette.surface || "#ffffff";
  const primary = palette.primary || "#2563EB";
  const accent = palette.accent || "#0EA5E9";
...
function buildPodcastCover(style, content, brand) {
  const palette = style?.palette || {};
  ...
  const images = [{ id: "img1", prompt: content.imagePrompt || "music performer portrait, neon lighting, textured background", url: heroUrl }];
  return { nodes, animations, images };
}
...
function pickImage(content = {}, brand = null) {
  if (brand?.logos?.length) return brand.logos[0];
  const placeholderImages = [
    "/packs/placeholders/cinematic-1.jpg",
    "/packs/placeholders/cinematic-2.jpg",
    "/packs/placeholders/cinematic-3.jpg",
    "/packs/placeholders/cinematic-4.jpg",
    "/packs/placeholders/cinematic-5.jpg",
  ];
  const seed = typeof content.imagePrompt === "string" ? content.imagePrompt.length : Math.random();
  const idx = Math.abs(Math.floor(seed % placeholderImages.length));
  return placeholderImages[idx] || placeholderImages[0];
}
