...
import { applyMotionThemeToLayers } from "@/engine/applyMotionTheme";
import { motionThemeMap } from "@/engine/motion/motionThemes";
...
export default function TemplateAIGenerator() {
  ...
  const [brandKit, setBrandKit] = useState(loadInitialBrandKit);
  const [brandName, setBrandName] = useState(() => brandKit?.name || "");
  const [brandTagline, setBrandTagline] = useState(() => brandKit?.tagline || "");
  const [brandPersonality, setBrandPersonality] = useState(() =>
    Array.isArray(brandKit?.personality) ? brandKit.personality.join(", ") : "",
  );
  const [brandPrimary, setBrandPrimary] = useState(() => brandKit?.theme?.tokens?.colors?.primary || "");
  const [brandSecondary, setBrandSecondary] = useState(() => brandKit?.theme?.tokens?.colors?.secondary || "");
  const [icons, setIcons] = useState([]);
  const [brandIcon, setBrandIcon] = useState(() => brandKit?.icon || brandKit?.iconUrl || "");
