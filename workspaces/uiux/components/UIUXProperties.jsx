...
import { useTextEditStore } from '@/runtime/stores/textEditStore';
import { SpanTypographyPanel } from '@/ui/panels/properties/typography/SpanTypographyPanel';
import { ComponentInstancePanel } from '@/ui/panels/properties/ComponentInstancePanel';
import { usePrototypeStore } from '@/runtime/stores/prototypeStore';
...
import GradientPicker from '@/ui/shared/GradientPicker';
...
const inputClass = 'w-full bg-white border border-neutral-200 rounded-md px-3 py-2 text-sm text-neutral-800 placeholder-neutral-400 focus:outline-none focus:ring-2 focus:ring-violet-200 focus:border-violet-500 transition';
const miniButton = 'px-2 py-1 rounded-md border border-neutral-200 bg-white text-xs font-semibold text-neutral-700 hover:border-violet-200 hover:bg-violet-50 transition';
const pillButton = (active) => `px-2.5 py-1 rounded-full text-xs font-semibold transition border ${active ? 'border-violet-200 bg-violet-50 text-violet-700' : 'border-neutral-200 bg-white text-neutral-700 hover:border-violet-200 hover:bg-violet-50'}`;
...
export default function UIUXProperties() {
    ...
    const selectionType = !selection ? 'Nothing Selected' : selection.type === 'text' ? 'Text' : selection.type === 'image' ? 'Image' : selection.type === 'frame' ? 'Frame' : selection.type === 'component' ? 'Component' : 'Layer';
    ...
    const frames = useMemo(() => Object.values(nodes).filter((n) => n.type === 'frame'), [nodes]);
    ...
    const hasTextSelection = selection?.type === 'richtext' && textEditingId === selection?.id && selectionStart !== null && selectionEnd !== null && selectionStart !== selectionEnd;
    const setFrameBackground = (bgPatch = {}) => {
        if (!selection) return;
        const next = {
            type: selection.background?.type || 'color',
            value: selection.background?.value || '#d1d5db',
            size: selection.background?.size || selection.backgroundSize || 'cover',
            position: selection.background?.position || selection.backgroundPosition || 'center',
            ...bgPatch,
        };
        updateNode(selection.id, { background: next });
    };
        ...
        <div className='p-4 space-y-4'>
            ...
            <Section title='Selection' defaultOpen>
                <div className='space-y-2'>
                    ...
                    {selection?.slot ? <div className='inline-flex items-center gap-2 rounded-full bg-violet-50 text-violet-700 border border-violet-200 px-3 py-1 text-[11px] font-semibold'>Slot: {selection.slotName || selection.slot}</div> : null}
                    <input className={inputClass} placeholder={selection?.name || 'Layer name'} defaultValue={selection?.name} onBlur={(e) => selection && updateNode(selection.id, { name: e.target.value })} />
                    {selection ? (
                ...
                {selection ? (
                    <div className='grid grid-cols-2 gap-2 mt-2'>
                        <div>
                            ...
                            <select className={inputClass} value={selection.slot || ''} onChange={(e) => updateNode(selection.id, { slot: e.target.value || null })}>
                        ...
                        <div>
                            ...
                            <input className={inputClass} placeholder='label, avatar, body...' value={selection.slotName || ''} onChange={(e) => updateNode(selection.id, { slotName: e.target.value })} />
                        </div>
                    </div>
                ) : null}
                {!selection ? <div className='rounded-md bg-neutral-50 border border-dashed border-neutral-200 px-3 py-2 text-xs text-neutral-500'>Select a frame, shape, or text layer to edit its properties.</div> : null}
            </Section>
            ...
            <Section title='Position & Size' defaultOpen>
                <div className='grid grid-cols-2 gap-2'>
                    <div>
                        ...
                        <input className={inputClass} placeholder='0' value={finiteOrEmpty(selection?.x)} onChange={(e) => selection && updateNode(selection.id, { x: parseFloat(e.target.value || '0') })} />
                    </div>
                    <div>
                        <div className={labelClass}>Y</div>
                        <input className={inputClass} placeholder='0' value={finiteOrEmpty(selection?.y)} onChange={(e) => selection && updateNode(selection.id, { y: parseFloat(e.target.value || '0') })} />
                    </div>
                    <div>
                        <div className={labelClass}>Width</div>
                        <input className={inputClass} placeholder='1440' value={finiteOrEmpty(selection?.width)} onChange={(e) => selection && updateNode(selection.id, { width: parseFloat(e.target.value || '0') })} />
                    </div>
                    <div>
                        <div className={labelClass}>Height</div>
                        <input className={inputClass} placeholder='900' value={finiteOrEmpty(selection?.height)} onChange={(e) => selection && updateNode(selection.id, { height: parseFloat(e.target.value || '0') })} />
                    </div>
                </div>
                <div className='grid grid-cols-3 gap-2'>
                    <div>
                        <div className={labelClass}>Rotation</div>
                        <input className={inputClass} placeholder='0°' value={finiteOrEmpty(selection?.rotation, 0)} onChange={(e) => selection && updateNode(selection.id, { rotation: parseFloat(e.target.value || '0') })} />
                    </div>
            ...
            {isFrame ? (
                <Section title='Frame Background' defaultOpen>
                    <div className='grid grid-cols-2 gap-2'>
                        <div>
                            ...
                            <input
                                ...
                                value={selection?.background?.type === 'color' ? selection.background.value || '#d1d5db' : '#d1d5db'}
                                onChange={(e) => setFrameBackground({ type: 'color', value: e.target.value })}
                            />
                        </div>
                        <div>
                            ...
                            <input
                                className={inputClass}
                                placeholder='https://...'
                                value={selection?.background?.type === 'image' ? selection.background.value || '' : ''}
                                onChange={(e) =>
                                    setFrameBackground(
                                        e.target.value
                    ...
                    <div className='grid grid-cols-2 gap-2 mt-2'>
                        <div>
                            ...
                            <GradientPicker
                                value={selection?.background?.type === 'gradient' ? selection.background.value || '' : ''}
                                onChange={(val) => setFrameBackground(val ? { type: 'gradient', value: val } : { type: 'none', value: null })}
                            />
                        </div>
                        <div>
                            <div className={labelClass}>Image Fit</div>
                            <select
                                className={inputClass}
                                value={selection?.background?.size || 'cover'}
                                onChange={(e) => setFrameBackground({ size: e.target.value })}>
            ...
            {isFrame ? (
                <Section title='Frame Stroke' defaultOpen={false}>
                    <div className='grid grid-cols-2 gap-2'>
                        <div>
                            <div className={labelClass}>Stroke Color</div>
                            <input type='color' className='w-full h-10 border border-neutral-200 rounded-md' value={selection?.stroke || '#000000'} onChange={(e) => updateNode(selection.id, { stroke: e.target.value })} />
                        </div>
                        <div>
                            <div className={labelClass}>Stroke Width</div>
                            <input className={inputClass} type='number' min='0' step='0.5' value={selection?.strokeWidth ?? 1} onChange={(e) => updateNode(selection.id, { strokeWidth: parseFloat(e.target.value || '0') })} />
                        </div>
                    </div>
                </Section>
            ) : null}

            {!isImage && selection && !isFrame ? (
                ...
                <Section title='Shape Layout' defaultOpen>
                    <div className='mt-2 grid grid-cols-2 gap-2'>
                        <div>
                            ...
                            <input className={inputClass} placeholder='https://...' value={selection?.backgroundImage || ''} onChange={(e) => updateNode(selection.id, { backgroundImage: e.target.value })} />
                        </div>
                        <div>
                            <div className={labelClass}>Gradient</div>
                            <GradientPicker value={selection?.backgroundGradient || ''} onChange={(val) => updateNode(selection.id, { backgroundGradient: val || null })} />
                        </div>
                    </div>
                    <div className='mt-2 grid grid-cols-3 gap-2'>
                        ...
                        <div>
                            ...
                            <input className={inputClass} type='number' min='0' value={selection?.padding ?? 0} onChange={(e) => updateNode(selection.id, { padding: parseInt(e.target.value || '0', 10) })} />
                        </div>
            ...
            {isImage && selection ? (
                <Section title='Image' defaultOpen>
                    <div className='space-y-2'>
                        <div>
                            <div className={labelClass}>Image URL</div>
                            <input className={inputClass} placeholder='https://...' value={selection?.src || selection?.url || ''} onChange={(e) => updateNode(selection.id, { src: e.target.value, url: e.target.value })} />
                        </div>
                        <div>
                            <div className={labelClass}>Fit</div>
                            <select className={inputClass} value={selection?.objectFit || 'cover'} onChange={(e) => updateNode(selection.id, { objectFit: e.target.value })}>
                                <option value='cover'>Cover</option>
                                <option value='contain'>Contain</option>
                                <option value='fill'>Fill</option>
                        ...
                        <div className='grid grid-cols-2 gap-2'>
                            <div>
                                <div className={labelClass}>Opacity</div>
                                <input className={inputClass} type='number' min='0' max='1' step='0.05' value={selection?.opacity ?? 1} onChange={(e) => updateNode(selection.id, { opacity: parseFloat(e.target.value || '1') })} />
                            </div>
                            <div>
                                <div className={labelClass}>Radius</div>
                                <input className={inputClass} type='number' min='0' step='2' value={selection?.borderRadius ?? 0} onChange={(e) => updateNode(selection.id, { borderRadius: parseFloat(e.target.value || '0') })} />
                            </div>
                        </div>
                        <div className='grid grid-cols-2 gap-2'>
                            <div>
                                <div className={labelClass}>Tint</div>
                                <input type='color' className='w-full h-10 border border-neutral-200 rounded-md' value={selection?.tint || '#ffffff'} onChange={(e) => updateNode(selection.id, { tint: e.target.value })} />
                            </div>
                            <div>
                                <div className={labelClass}>Blur</div>
                                <input className={inputClass} type='range' min='0' max='40' step='1' value={selection?.blur || 0} onChange={(e) => updateNode(selection.id, { blur: parseFloat(e.target.value || '0') })} />
                ...
                <Section title='Vector' defaultOpen>
                    ...
                    <div className='grid grid-cols-2 gap-2'>
                        <div>
                            ...
                            <select className={inputClass} value={selection?.strokeLinecap || 'round'} onChange={(e) => updateNode(selection.id, { strokeLinecap: e.target.value })}>
                        ...
                        <div>
                            ...
                            <select className={inputClass} value={selection?.strokeLinejoin || 'round'} onChange={(e) => updateNode(selection.id, { strokeLinejoin: e.target.value })}>
                    ...
                    <div className='mt-2'>
                        ...
                        <input className={inputClass} placeholder='e.g. 4 2' value={selection?.strokeDasharray || ''} onChange={(e) => updateNode(selection.id, { strokeDasharray: e.target.value })} />
                    </div>
                </Section>
            ) : null}
            ...
            <Section title='Constraints & Layout' defaultOpen>
                <div className='grid grid-cols-2 gap-2'>
                    <button className={pillButton(layout.enabled)} onClick={() => selection && updateNode(selection.id, { layout: { ...(selection.layout || {}), enabled: !layout.enabled } })}>
                ...
                {layout.enabled ? (
                    <div className='grid grid-cols-2 gap-2 mt-2'>
                        ...
                        <label className='flex items-center gap-2 text-sm'>
                            <input type='checkbox' checked={layout.wrap || false} onChange={(e) => selection && updateNode(selection.id, { layout: { ...(selection.layout || {}), wrap: e.target.checked } })} />
                ...
                <div className='grid grid-cols-2 gap-2'>
                    <select className={inputClass} value={constraints.vertical || 'top'} onChange={(e) => selection && updateNode(selection.id, { constraints: { ...(constraints || {}), vertical: e.target.value } })}>
                ...
                <div className='grid grid-cols-2 gap-2'>
                    <select className={inputClass} value={layout.direction || 'vertical'} onChange={(e) => selection && updateNode(selection.id, { layout: { ...(selection.layout || {}), enabled: true, direction: e.target.value } })} disabled={!layout.enabled}>
                        <option value='horizontal'>Horizontal</option>
                        <option value='vertical'>Vertical</option>
                    </select>
                    <select className={inputClass} value={layout.alignment || 'start'} onChange={(e) => selection && updateNode(selection.id, { layout: { ...(selection.layout || {}), enabled: true, alignment: e.target.value } })} disabled={!layout.enabled}>
                ...
                <div className='grid grid-cols-3 gap-2'>
                    <button className={pillButton(layout.primaryAxisSizing === 'hug')} onClick={() => selection && updateNode(selection.id, { layout: { ...(selection.layout || {}), enabled: true, primaryAxisSizing: 'hug' } })} disabled={!layout.enabled}>
                        Hug
                    </button>
                    <button className={pillButton(layout.primaryAxisSizing === 'fill')} onClick={() => selection && updateNode(selection.id, { layout: { ...(selection.layout || {}), enabled: true, primaryAxisSizing: 'fill' } })} disabled={!layout.enabled}>
                        Fill
                    </button>
                    <button className={pillButton(layout.primaryAxisSizing === 'fixed' || !layout.primaryAxisSizing)} onClick={() => selection && updateNode(selection.id, { layout: { ...(selection.layout || {}), enabled: true, primaryAxisSizing: 'fixed' } })} disabled={!layout.enabled}>
                        Fixed
                    </button>
                </div>
            </Section>
            ...
            <Section title='Appearance' defaultOpen>
                <div className='grid grid-cols-2 gap-2'>
                    <div>
                        ...
                        <input className={inputClass} placeholder='Solid / Gradient' defaultValue={selection?.fill} onBlur={(e) => selection && updateNode(selection.id, { fill: e.target.value })} />
                    </div>
                    <div>
                        <div className={labelClass}>Stroke</div>
                        <input className={inputClass} placeholder='Color' defaultValue={selection?.stroke} onBlur={(e) => selection && updateNode(selection.id, { stroke: e.target.value })} />
                    </div>
                </div>
                <div className='grid grid-cols-2 gap-2'>
                    <input className={inputClass} placeholder='Stroke Width' defaultValue={selection?.strokeWidth} onBlur={(e) => selection && updateNode(selection.id, { strokeWidth: parseFloat(e.target.value || '0') || 0 })} />
                    <input className={inputClass} placeholder='Opacity %' defaultValue={selection?.opacity ? selection.opacity * 100 : 100} onBlur={(e) => selection && updateNode(selection.id, { opacity: Math.max(0, Math.min(1, (parseFloat(e.target.value) || 100) / 100)) })} />
                </div>
            </Section>
            ...
            <Section title='3D Transform' defaultOpen={false}>
                ...
                <div className='grid grid-cols-2 gap-2'>
                    <select className={inputClass} value={transform3d.transformStyle || 'flat'} onChange={(e) => selection && updateNode(selection.id, { transform3d: { ...transform3d, transformStyle: e.target.value } })}>
            ...
            {isText && hasTextSelection && selection ? (
                ...
                <Section title='Typography' defaultOpen={false}>
                    <div className='grid grid-cols-2 gap-2'>
                        <input className={inputClass} placeholder='Font Family' defaultValue={selection?.fontFamily} onBlur={(e) => selection && updateNode(selection.id, { fontFamily: e.target.value })} />
                        <select className={inputClass} defaultValue={selection?.fontWeight || '400'} onChange={(e) => selection && updateNode(selection.id, { fontWeight: e.target.value })}>
                    ...
                    <div className='grid grid-cols-3 gap-2'>
                        <input className={inputClass} placeholder='Size' defaultValue={selection?.fontSize || 16} onBlur={(e) => selection && updateNode(selection.id, { fontSize: parseFloat(e.target.value || '0') })} />
                            ...
                            <div className='space-y-3 mt-3'>
                                {slots.map((slot) => (
                                    <div key={slot.id} className='border border-neutral-200 rounded-md p-3 space-y-2'>
                                        ...
                                        {slot.slot === 'text' ? <input className={inputClass} placeholder={slot.text || 'Override text'} value={selection.nodeOverrides?.[`${slot.id}.text`] || ''} onChange={(e) => setOverride(slot.id, 'text', e.target.value)} /> : null}
                                        {slot.slot === 'text' ? <input className={inputClass} type='color' value={selection.nodeOverrides?.[`${slot.id}.fill`] || slot.fill || '#ffffff'} onChange={(e) => setOverride(slot.id, 'fill', e.target.value)} title='Text color' /> : null}
                                        {slot.slot === 'image' || slot.slot === 'icon' ? <input className={inputClass} placeholder={slot.src || 'Image URL'} value={selection.nodeOverrides?.[`${slot.id}.src`] || ''} onChange={(e) => setOverride(slot.id, 'src', e.target.value)} /> : null}
                                        {slot.slot === 'container' || slot.slot === 'custom' ? <input className={inputClass} type='color' value={selection.nodeOverrides?.[`${slot.id}.fill`] || slot.fill || '#ffffff'} onChange={(e) => setOverride(slot.id, 'fill', e.target.value)} /> : null}
                                        <button
                                            className={miniButton}
                                            onClick={() => {
                ...
                <Section title='Interactions' defaultOpen={false}>
                    <div className='space-y-2'>
                            ...
                            <div key={`${interaction.trigger}-${idx}`} className='rounded-md border border-neutral-200 bg-white px-3 py-2 space-y-2'>
                                <div className='grid grid-cols-2 gap-2'>
                                    <select
                                        ...
                                        onChange={(e) =>
                                            updateNode(selection.id, {
                                                interactions: (selection.interactions || []).map((i, ii) => (ii === idx ? { ...i, trigger: e.target.value } : i)),
                                            })
                                        }>
                                        {triggerList.map((t) => (
                        ...
                        <button
                            ...
                            onClick={() =>
                                updateNode(selection.id, {
                                    interactions: [...(selection.interactions || []), { trigger: 'onClick', actions: [{ type: 'navigate', target: frames[0]?.id || '' }] }],
                                })
                            }>
                            + Add Interaction
                        </button>
