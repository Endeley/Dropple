...
import { ASSET_CATALOG, useAssetBrowserStore } from '@/runtime/stores/assetBrowserStore';
...
const TEMPLATE_PLACEHOLDER = 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&w=1200&q=80';
...
const Tag = ({ label }) => <span className='inline-flex items-center px-2 py-1 text-[11px] font-semibold rounded-full bg-white/80 text-neutral-700 border border-white/70 shadow-sm'>{label}</span>;
...
export default function AssetBrowser() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                const [tRes, pRes, rRes] = await Promise.all([fetch('/api/templates/trending', { signal: controller.signal }), fetch('/api/templates/popular', { signal: controller.signal }), fetch('/api/templates/recent', { signal: controller.signal })]);
    ...
    useEffect(() => {
        const loadDetailData = async () => {
            ...
            try {
                const [sRes, recRes] = await Promise.all([fetch(`/api/templates/stats?id=${selectedTemplate.id}`), fetch('/api/templates/recommended')]);
    ...
    const catalogForTab = useMemo(() => {
        ...
        return ASSET_CATALOG.filter((item) => {
            const matchTab = (tab === 'assets' && item.type === 'asset') || (tab === 'icons' && item.type === 'icon') || (tab === 'components' && item.type === 'component') || (tab === 'cloud' && item.type === 'asset');
            return matchTab;
        });
    }, [tab, templates, components, assets, myTemplates]);

    const defaultCategories = ['All', 'Website', 'Mobile', 'Hero Sections', 'Dashboard', 'E-commerce', 'Portfolio', 'Marketing', 'Wireframes', 'Presentations', 'Branding', 'Social Media'];
    ...
    const hasActiveFilters = (search && search.trim().length > 0) || category !== 'All' || layoutFilter !== 'All' || styleFilter !== 'All' || colorFilter !== 'All';
        ...
        <div
            ...
            <div className='relative h-40 w-full bg-neutral-100'>
                {item.previewUrl || item.thumbnail || item.thumbnailUrl ? (
                    <Image src={item.previewUrl || item.thumbnail || item.thumbnailUrl || TEMPLATE_PLACEHOLDER} alt={item.name} fill className='object-cover' sizes='320px' />
                ) : (
                ...
                {item.tags?.length ? (
                    <div className='flex flex-wrap gap-1'>
                        {item.tags.slice(0, 3).map((t) => (
                            <span key={t} className='text-[10px] px-2 py-0.5 rounded-full bg-neutral-100 text-neutral-600'>
                                #{t}
                            </span>
                        ))}
                    </div>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm'>
                    ...
                    <div className='flex items-center gap-2'>
                        {tabs.map((t) => (
                            <button
                                key={t.id}
                                onClick={() => setTab(t.id)}
                                className={`px-3 py-1.5 rounded-full text-sm font-semibold transition border ${tab === t.id ? 'bg-violet-500/10 border-violet-500 text-violet-700' : 'bg-white border-neutral-200 text-neutral-700 hover:border-violet-200 hover:bg-violet-50'}`}>
                                {t.label}
                            </button>
                        ))}
                    </div>
                    <button onClick={close} className='text-sm font-semibold text-neutral-600 hover:text-neutral-900 px-3 py-1 rounded-md hover:bg-neutral-100'>
                        Close
                    </button>
                </div>
                    ...
                    <div className='flex flex-col gap-2'>
                        ...
                        <div className='flex items-center gap-2 overflow-x-auto'>
                            {categories.map((c) => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-full text-xs font-semibold transition border ${category === c ? 'bg-violet-500/10 border-violet-500 text-violet-700' : 'bg-white border-neutral-200 text-neutral-700 hover:border-violet-200 hover:bg-violet-50'}`}>
                                    {c}
                                </button>
                            ))}
                        </div>
                        {tab === 'templates' ? (
                            <div className='flex items-center gap-2'>
                                ...
                                <select className='border border-neutral-200 rounded-md px-2 py-1 text-xs text-neutral-700 bg-white' value={sort} onChange={(e) => setSort(e.target.value)}>
                        ...
                        {tab === 'templates' ? (
                            <div className='flex flex-col gap-1'>
                                <div className='flex items-center gap-2'>
                                    <button className='px-3 py-1.5 rounded-md border border-neutral-200 text-xs text-neutral-700 hover:bg-neutral-100' onClick={clearFilters}>
                                        Clear filters
                                    </button>
                                    {hasActiveFilters ? <span className='text-[11px] text-neutral-500'>Filters active</span> : null}
                                </div>
                                <div className='flex items-center gap-2 overflow-x-auto'>
                                    ...
                                    {layoutOptions.map((opt) => (
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded-full text-xs font-semibold transition border ${layoutFilter === opt ? 'bg-violet-500/10 border-violet-500 text-violet-700' : 'bg-white border-neutral-200 text-neutral-700 hover:border-violet-200 hover:bg-violet-50'}`}>
                                            {opt}
                                        </button>
                                    ))}
                                </div>
                                <div className='flex items-center gap-2 overflow-x-auto'>
                                    ...
                                    {styleOptions.map((opt) => (
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded-full text-xs font-semibold transition border ${styleFilter === opt ? 'bg-violet-500/10 border-violet-500 text-violet-700' : 'bg-white border-neutral-200 text-neutral-700 hover:border-violet-200 hover:bg-violet-50'}`}>
                                            {opt}
                                        </button>
                                    ))}
                                </div>
                                <div className='flex items-center gap-2 overflow-x-auto'>
                                    ...
                                    {colorOptions.map((opt) => (
                                        <button
                                            ...
                                            className={`px-3 py-1.5 rounded-full text-xs font-semibold transition border ${colorFilter === opt ? 'bg-violet-500/10 border-violet-500 text-violet-700' : 'bg-white border-neutral-200 text-neutral-700 hover:border-violet-200 hover:bg-violet-50'}`}>
                                            {opt}
                                        </button>
                                    ))}
                                </div>
                        ...
                        <div className='space-y-4'>
                            {recommended.length ? (
                                <div className='mb-2'>
                                    ...
                                    <div className='grid grid-cols-2 gap-3'>{recommended.slice(0, 4).map((item) => templateCard(item))}</div>
                                </div>
                            ) : null}
                            {trending.length ? (
                                <div className='mb-2'>
                                    <div className='text-sm font-semibold text-neutral-800 mb-2'>Trending</div>
                                    <div className='grid grid-cols-2 gap-3'>{trending.slice(0, 4).map((item) => templateCard(item))}</div>
                                </div>
                            ) : null}
                            {popular.length ? (
                                <div className='mb-2'>
                                    <div className='text-sm font-semibold text-neutral-800 mb-2'>Popular</div>
                                    <div className='grid grid-cols-2 gap-3'>{popular.slice(0, 4).map((item) => templateCard(item))}</div>
                                </div>
                            ) : null}
                            {recent.length ? (
                                <div className='mb-2'>
                                    <div className='text-sm font-semibold text-neutral-800 mb-2'>Recent</div>
                                    <div className='grid grid-cols-2 gap-3'>{recent.slice(0, 4).map((item) => templateCard(item))}</div>
                                </div>
                            ) : null}
                            <div className='grid grid-cols-2 gap-4'>{filtered.map((item) => templateCard(item))}</div>
                        </div>
                    )}
                </div>
            </div>
            ...
            {selectedTemplate ? (
                <TemplateDetailModal
                    ...
                    onInsert={() => {
                        if (selectedTemplate.type === 'template' && !selectedTemplate.isLocal) logTemplateEvent(selectedTemplate.id, 'insert');
            ...
            {aiTargetTemplate ? <TemplateAIAdvancedModal template={aiTargetTemplate} onClose={() => setAiTargetTemplate(null)} onDone={() => setAiTargetTemplate(null)} /> : null}
        </div>
    );
}
