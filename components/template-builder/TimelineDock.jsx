...
export default function TimelineDock() {
  ...
  const activePage =
    pages.find((p) => p.id === activePageId) || pages[0] || { layers: [] };

  const timelineLayers = useMemo(
    () =>
      (activePage.layers || []).filter((l) => selectedLayers.includes(l.id)),
  ...
  const addKeyframeAtPlayhead = () => {
    ...
    const layer =
      active?.layers?.find((l) => state.selectedLayers.includes(l.id)) || null;
    ...
    <div className="border-t bg-slate-50">
      <div className="flex flex-wrap items-center justify-between gap-3 px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setTimelineLoop(!loop)}
            className={`flex items-center gap-2 rounded border px-3 py-2 text-xs font-medium ${
              loop
          ...
          <div className="flex items-center gap-2 rounded border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700">
            <span className="font-semibold">Time</span>
            <input
              type="number"
              className="w-20 rounded border border-slate-200 px-2 py-1 text-xs"
              value={Math.round(scrubberTime)}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-1 text-slate-600">
            <button
              onClick={() => setTimelineZoom((zoom || 1) - 0.25)}
              className="rounded border border-slate-300 p-1 hover:bg-white"
            >
              <ZoomOut size={14} />
            </button>
          ...
          <button
            ...
            className="flex items-center gap-2 rounded border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800 hover:bg-amber-100"
          >
            <Plus size={14} />
            Add Keyframe
          </button>
          <button
            onClick={() => ensureTimelineForSelection()}
            className="flex items-center gap-2 rounded border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50"
          >
            <CirclePlus size={14} />
            Add Track
          </button>
      ...
      <div className="h-[260px] overflow-x-auto border-t border-slate-200 bg-white">
        {!timelineLayers.length ? (
          <div className="flex h-full items-center justify-center text-sm text-slate-500">
            Select a layer to view its timeline, then click “Tracks” to initialize motion.
          </div>
        ) : (
          <div
            ref={containerRef}
          ...
          >
            ...
            <div
              ...
              style={{ left: timeToPx(scrubberTime) + LABEL_WIDTH }}
            />
            <div
              className="pointer-events-none absolute top-0 -translate-x-1/2 rounded-b bg-rose-500 px-2 py-1 text-[10px] font-semibold text-white shadow"
              style={{ left: timeToPx(scrubberTime) + LABEL_WIDTH }}
            >
              {formatTimecode(scrubberTime)}
            </div>
            ...
            <div className="sticky left-0 z-10 flex h-8 bg-slate-100 text-[11px] text-slate-600">
              <div
                className="border-r border-slate-200 px-3 py-2 font-semibold uppercase tracking-wide"
              ...
              <div className="relative flex-1">
                {markers.map((ms) => (
                  <div
                    key={ms}
                    className="absolute top-0 h-8 border-r border-slate-200 px-1"
                    style={{ left: timeToPx(ms) }}
                  >
                    <span className="absolute top-0 text-[10px] text-slate-500">
                      {ms}ms
                    </span>
                  </div>
                ))}
            ...
            <div className="relative">
              {timelineLayers.map((layer) => {
                ...
                const layerLabel =
                  layer.name || layer.type || layer.id || "Layer";
                return (
                  <div key={layer.id} className="border-t border-slate-100">
                    <div className="flex items-center justify-between bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-700">
                      <div className="flex items-center gap-3">
                        <span className="rounded bg-slate-900 px-2 py-1 text-[11px] font-mono text-white">
                          {layer.id.slice(0, 6)}
                        </span>
                      ...
                      return (
                        <TrackRow
                          ...
                          onAddKeyframe={(time, value) =>
                            addKeyframe(layer.id, animation?.id, track.property, time, value)
                          }
                          onUpdateKeyframe={(idx, time) =>
                            updateKeyframeTime(layer.id, animation?.id, track.property, idx, time)
                          }
...
function TrackRow({
  ...
  return (
    <div
      ...
      style={{ gridTemplateColumns: `${LABEL_WIDTH}px 1fr`, minHeight: ROW_HEIGHT }}
    >
...
function KeyframeDot({ left, easing, onMove, color }) {
  ...
  return (
    <div
      className="absolute top-1/2 z-10 h-3 w-3 -translate-y-1/2 rotate-45 cursor-ew-resize shadow ring-1 ring-slate-900/10 hover:scale-110"
      style={{ left, background: color || "#fbbf24", border: "1px solid #0f172a22" }}
      title={easing || "linear"}
