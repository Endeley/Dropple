...
export default function BuilderSidebar() {
    ...
    return (
        <div className='w-72 h-full border-r border-slate-200 bg-white shadow-sm flex flex-col text-gray-900'>
            <div className='grid grid-cols-2 gap-2 px-3 py-2 bg-slate-50 border-b border-slate-200'>
                {['layers', 'assets', 'components', 'themes'].map((key) => (
                    <button
                        key={key}
                        onClick={() => setTab(key)}
                        className={`px-3 py-2 rounded-md text-xs font-semibold capitalize transition whitespace-nowrap ${tab === key ? 'bg-white text-slate-900 shadow-stone-800 border border-slate-200' : 'text-slate-600 hover:bg-white/70 border border-transparent'}`}>
                        {key}
                    </button>
                ))}
            </div>
